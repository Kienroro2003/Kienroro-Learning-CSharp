...
.First();
        ...
        {
            //string messagePlayer = $"{player.Name} asked {playerToAsk.Name} for {valueToAskFor}{((valueToAskFor == Values.Six) ? "es" : "s")}";
            //var cardsOtherPlayerHave = playerToAsk.DoYouHaveAny(valueToAskFor, stock);
            //string messageOtherPlayer = string.Empty;
            //if (cardsOtherPlayerHave.Count() > 0)
            //{
            //    player.AddCardsAndPullOutBooks(cardsOtherPlayerHave);
            //    messageOtherPlayer = $"{playerToAsk.Name} has {cardsOtherPlayerHave.Count()} {valueToAskFor} card{Player.S(cardsOtherPlayerHave.Count())}";
            //}
            //else if(cardsOtherPlayerHave.Count() == 0 && stock.Count() > 0)
            //{
            //    player.DrawCard(stock);
            //}


            //string result = messagePlayer + Environment.NewLine + messageOtherPlayer;
            //return result;
